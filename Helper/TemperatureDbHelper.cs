...
{
    public class TemperatureDbHelper
    {
        string connectionString;
        public TemperatureDbHelper()
        {
        ...
        private async Task<ResponseObjectById> GetObjectById(Int64 idValore)
        {
            ...
            try
            {
                ...
                var query = string.Format(Queries.TEMPERATURE_GET_BY_ID, idValore);
        ...
        public async Task<ResponseObject> GetAll()
        {
            ...
            try
            {
                ...
                await using var dataSource = NpgsqlDataSource.Create(connectionString);
                await using var command = dataSource.CreateCommand(Queries.TEMPERATURE_GET);
                await using var rdr = await command.ExecuteReaderAsync();
        ...
        public async Task<ResponseObject> GetLastHour()
        {
            ...
            try
            {
                ...
                var query = string.Format(Queries.TEMPERATURE_GET_LAST_HOUR, dtInizio, dtFine);
        ...
        public async Task<ResponseObject> GetLast()
        {
            ...
            try
            {
                ...
                await using var dataSource = NpgsqlDataSource.Create(connectionString);
                await using var command = dataSource.CreateCommand(Queries.TEMPERATURE_GET_LAST);
                await using var rdr = await command.ExecuteReaderAsync();
        ...
        public async Task<ResponseObjectInsert> Insert(RequestObject request)
        {
            ...
            try
            {
                response.status = 1;
                response.message = "success";
                var query = string.Format(Queries.TEMPERATURE_POST_DATA, request.valore);
        ...
        public async Task<ResponseObject> GetShowData(int recordNumber)
        {
            ...
            try
            {
                response.status = 1;
                response.message = "success";
                var query = string.Format(Queries.TEMPERATURE_GET_SHOWDATA, recordNumber);
